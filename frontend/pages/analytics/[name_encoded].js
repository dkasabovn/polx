...
export default function SenatorStats(props) {
  ...
  const redOrGreenMoney = (number) => {
    if (number > 0) {
      return (
        <span className="text-green-500">{number.toLocaleString('en-US', { style: "currency", currency: 'USD' })}</span>
      )
    } else {
      return (
        <span className="text-red-500">{number.toLocaleString('en-US', { style: "currency", currency: 'USD' })}</span>
      )
    }
  }
              ...
              <div className="mb-10" key={i}>
                ...
                <div className="grid grid-cols-6 gap-4">
                  <div className="col-span-2 text-center shadow-xl px-5 py-5 rounded-lg bg-white">
                    ...
                    <div className="text-green-500 text-5xl font-bold">{redOrGreen(obj.shareDelta)}</div>
                  </div>
                  <div className="col-span-2 text-center shadow-xl px-5 py-5 rounded-lg bg-white">
